...
const SYMBOL = "DEMO:BTCUSD";

function resolutionToMs(resolution: string | Resolution): number {
	const r = typeof resolution === "string" ? resolution : String(resolution);
	const minute = 60_000;
...
// Deterministic-ish random walk so the chart looks like a real instrument.
function generateBars(from: number, to: number, stepMs: number): Bar[] {
	const bars: Bar[] = [];
	let price = 60_000;
...
export function createMockDatafeed(): Datafeed {
	return {
		resolveSymbol(
			symbolName: string,
			...
			// Resolve asynchronously, as a real network datafeed would.
